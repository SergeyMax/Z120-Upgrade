
#include "tef6686.h"

// all read radio commans will be transmitted with the index=0 for correct timings 
// as described in the paragraph 5.6 of the User Manual
const uint8_t get_op_status[]                   =   {0x40, 0x80, 0x00};
const uint8_t init_stop[]                       =   {0x1C, 0x00, 0x00};
const uint8_t init_dsp[]                        =   {0x1C, 0x00, 0x74};
const uint8_t init_lut[]                        =   {0x1C, 0x00, 0x75};
const uint8_t start[]                           =   {0x14, 0x00, 0x01};
const uint8_t set_crystal[]                     =   {0x40, 0x04, 0x01, 0x00, 0xB7, 0x1B, 0x00, 0x00, 0x00}; // 12 MHz
const uint8_t activate[]                        =   {0x40, 0x05, 0x01, 0x00, 0x01};

const uint8_t set_mute_off[]                    =   {0x30, 0x0B, 0x01, 0x00, 0x00};
const uint8_t set_stereo[]                      =   {0x20, 0x42, 0x01, 0x00, 0x00, 0x01, 0x90}; // separation not limited
const uint8_t set_mono[]                        =   {0x20, 0x42, 0x01, 0x00, 0x02, 0x01, 0x90}; // forced mono
const uint8_t enable_mph_suppression[]          =   {0x20, 0x14, 0x01, 0x00, 0x01};
const uint8_t enable_channel_equalizer[]        =   {0x20, 0x16, 0x01, 0x00, 0x01};
const uint8_t disable_mph_suppression[]         =   {0x20, 0x14, 0x01, 0x00, 0x00};
const uint8_t disable_channel_equalizer[]       =   {0x20, 0x16, 0x01, 0x00, 0x00};

const uint8_t get_quality_status[]              =   {0x20, 0x80, 0x00};
const uint8_t get_quality_data[]                =   {0x20, 0x81, 0x00};
const uint8_t get_signal_status[]               =   {0x20, 0x85, 0x00};
const uint8_t get_processing_status[]           =   {0x20, 0x86, 0x00};

const uint8_t set_rds[]                         =   {0x20, 0x51, 0x01, 0x00, 0x01, 0x00, 0x02, 0x00, 0x00}; // decoder mode
const uint8_t get_rds_data[]                    =   {0x20, 0x83, 0x00};

uint8_t tune_to[]                               =   {0x20, 0x01, 0x01, 0x00, 0x01, 0x29, 0x7C}; // 106.2MHz
uint8_t radio_response[16];
HAL_StatusTypeDef status;

extern I2C_HandleTypeDef hi2c1;

const uint8_t patch[] = // V102 P2.22
{
	
};

const uint8_t lut[] =
{
	0x40,0x13,0x40,0x2F,0x41,0x68,0x41,0xC1,0x42,0x14,0x44,0xDE,0x44,0xE9,0x45,0xFB,0x46,0x11,0x47,0xC5,0x47,0xFC,0x4D,0x5D,
	0x4E,0x56,0x4E,0x58,0x4E,0x5B,0x4D,0x83,0x4E,0x0A,0x4E,0x0B,0x4E,0x49,0x4E,0x53,0x4F,0x92,0x4F,0xEA,0x50,0x41,0x50,0x74,
	0x50,0x80,0x56,0x60,0x56,0xD4,0x56,0xD9,0x58,0x29,0x59,0xB4,0x5A,0x3A,0x5B,0x79,0x5B,0xA2,0x5B,0xEF,0x5D,0xDC,0x60,0x3A,
	0x60,0x88,0x60,0xA8,0x61,0x9F,0x61,0xB6,0x61,0xDF,0x62,0x14,0x62,0x59,0x62,0x9B,0x62,0xA1,0x63,0x69,0x64,0x3B,0x66,0x09,
	0x66,0xE7,0x67,0x0B,0x67,0x1A,0x67,0x29,0x68,0x87,0x68,0x99,0x68,0xA7,0x68,0xB2
};

HAL_StatusTypeDef radio_request( const uint8_t *request_data, uint8_t request_size, uint8_t *response_data, uint8_t response_size )
{
  HAL_StatusTypeDef status = HAL_ERROR;
  
  if( request_data && request_size )
    status = HAL_I2C_Master_Transmit( &hi2c1, RADIO_REQUEST_ADDRESS, (uint8_t*) request_data, request_size, RADIO_IO_TIMEOUT );

  if( response_data && response_size && status == HAL_OK )
  {
    status = HAL_I2C_Master_Receive( &hi2c1, RADIO_RESPONSE_ADDRESS, response_data, response_size, RADIO_IO_TIMEOUT );

    // There is a bug in the HAL_I2C_Master_Receive(). This function can return fake HAL_TIMEOUT error if there was a small 
    // delay (e.g. systick interrupt) between RXNE and STOPF polling in the I2C_WaitOnRXNEFlagUntilTimeout().
    // To work it around we need to read RX byte, place it to the last byte of the response_data, and safely ignore the error
    if( status == HAL_TIMEOUT &&  HAL_I2C_GetError(&hi2c1) == HAL_I2C_ERROR_NONE )
    {
      response_data[response_size-1] = hi2c1.Instance->RXDR;
      status = HAL_OK;
    }
  }

  return( status );
}

void radio_send_patch( const uint8_t* patch, uint32_t patch_size )
{
  uint8_t slice[RADIO_SLICE_SIZE + 1];
  slice[0] = 0x1B;

  uint32_t patch_position = 0;
  while( patch_position < patch_size )
  {
    uint32_t slice_size = patch_size - patch_position;
    if( slice_size > RADIO_SLICE_SIZE ) slice_size = RADIO_SLICE_SIZE;

    for( uint32_t i = 0; i < slice_size; i++ )
      slice[i + 1] = patch[patch_position + i];
    
    status = radio_request( slice, slice_size + 1, NULL, 0 );
    patch_position += slice_size;
  } 
}

void radio_init()
{
  while( radio_get_status() != 0x0000 ) // awaiting for the boot state
    HAL_Delay( 5 );
  
  status = radio_request( init_stop, sizeof(init_stop), NULL, 0 );
  status = radio_request( init_dsp, sizeof(init_dsp), NULL, 0 );

  radio_send_patch( patch, sizeof(patch) );

  status = radio_request( init_stop, sizeof(init_stop), NULL, 0 );
  status = radio_request( init_lut, sizeof(init_lut), NULL, 0 );

  radio_send_patch( lut, sizeof(lut) );

  status = radio_request( init_stop, sizeof(init_stop), NULL, 0 );
  status = radio_request( start, sizeof(start), NULL, 0 );
  
  while( radio_get_status() != 0x0001 ) // awaiting for the idle state
    HAL_Delay( 5 );

  status = radio_request( set_crystal, sizeof(set_crystal), NULL, 0 );
  status = radio_request( activate, sizeof(activate), NULL, 0 );

  while( radio_get_status() != 0x0002 ) // awaiting for the standby state
    HAL_Delay( 5 );

  status = radio_request( set_mute_off, sizeof(set_mute_off), NULL, 0 );
//  status = radio_request( enable_mph_suppression, sizeof(enable_mph_suppression), NULL, 0 );
//  status = radio_request( enable_channel_equalizer, sizeof(enable_channel_equalizer), NULL, 0 );
  status = radio_request( set_rds, sizeof(set_rds), NULL, 0 );
}

void radio_set_freq( uint16_t frequency )
{
  tune_to[5] = ( frequency >> 8 ) & 0xFF;
  tune_to[6] = frequency & 0xFF;
  status = radio_request( tune_to, sizeof(tune_to), NULL, 0 );
}

void radio_set_stereo( bool stereo )
{
  if( stereo )
    status = radio_request( set_stereo, sizeof(set_stereo), NULL, 0 );
  else
    status = radio_request( set_mono, sizeof(set_mono), NULL, 0 );
}

void radio_set_improvement( bool improve )
{
  if( improve )
  {
    status = radio_request( enable_mph_suppression, sizeof(enable_mph_suppression), NULL, 0 );
    status = radio_request( enable_channel_equalizer, sizeof(enable_channel_equalizer), NULL, 0 );
  }
  else
  {
    status = radio_request( disable_mph_suppression, sizeof(disable_mph_suppression), NULL, 0 );
    status = radio_request( disable_channel_equalizer, sizeof(disable_channel_equalizer), NULL, 0 );
  }
}

void radio_get_rds_data()
{
  status = radio_request( get_rds_data, sizeof(get_rds_data), radio_response, 14 );
}

bool radio_station_detected()
{
  status = radio_request( get_quality_data, sizeof(get_quality_data), radio_response, 16 );
  
  if( swap(&radio_response[4]) < RADIO_LEVEL_THRESHOLD )
    return false;
  
  if( swap(&radio_response[6]) > RADIO_USN_THRESHOLD )
    return false;
  
  if( swap(&radio_response[8]) > RADIO_WAM_THRESHOLD )
    return false;

  if( abs((int16_t)swap(&radio_response[10])) > RADIO_OFFSET_THRESHOLD )
    return false;
  
  return true;
}

uint16_t radio_get_status()
{
  status = radio_request( get_op_status, sizeof(get_op_status), radio_response, 4 );
  return( swap(&radio_response[2]) );
}

uint16_t radio_get_quality_status()
{
  status = radio_request( get_quality_status, sizeof(get_quality_status), radio_response, 4 );
  return( swap(&radio_response[2]) );
}

uint16_t radio_get_signal_status()
{
  status = radio_request( get_signal_status, sizeof(get_signal_status), radio_response, 4 );
  return( swap(&radio_response[2]) );
}

uint16_t radio_get_processing_status()
{
  status = radio_request( get_processing_status, sizeof(get_processing_status), radio_response, 10 );
  return( swap(&radio_response[6]) );
}

